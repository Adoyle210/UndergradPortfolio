...
use std::thread; //for thread portion of assignment
//use std::sync::Mutex; //locking and unlocking threads
...
fn main() {
    ...
    //changed:
    //used: https://doc.rust-lang.org/book/ch16-01-threads.html
    //used: https://doc.rust-lang.org/std/clone/trait.Clone.html
  
    /*for i in 0..1{ //still two threads i just looped it dont come for me pls 
      let xs_clone = xs[i].clone(); //make clones to move
      let t = thread::spawn(move || map_data(&xs_clone));//making threads
      let r = t.join().unwrap(); 
      intermediate_sums.push(r); //adding data to intermediate_sums
    } //jk this dont work be of r and it needs to be two loops */

    let xs_clone = xs[0].clone(); //make clones to move
    let xs_clone1 = xs[1].clone(); //make clones to move
    let t = thread::spawn(move || map_data(&xs_clone));//making threads
    let t1 = thread::spawn(move || map_data(&xs_clone1));//making threads
    let r = t.join().unwrap(); 
    let r1 = t1.join().unwrap(); 
    intermediate_sums.push(r); //adding data to intermediate_sums
    intermediate_sums.push(r1); //adding data to intermediate_sums
    ...
    /**********************************************************
    * NOTE: I MODELED AFTER THE FIRST ONE....
    * used:https://doc.rust-lang.org/std/thread/struct.JoinHandle.html
    * used: https://doc.rust-lang.org/std/clone/trait.Clone.html
    *************************************************************/
    ...
    for x in 0..num_partitions {
      let xs2_clone = xs2[x].clone(); //make clone to move
      threads.push(thread::spawn(move || map_data(&xs2_clone)));
    }

    // 4. Collects the intermediate sums from all the threads
    for _y in 0..num_partitions {
      let returns = threads.remove(0).join().unwrap(); //get returns
      intermediate_sums2.push(returns); //push returns to sums
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
  //THE FOLLOWING IS MODELED AFTER PARTITION DATA IN TWO KINDA:
   let mut xs: Vec<Vec<usize>> = Vec::new(); //vector for ints
   let mut index = 0;
   ...
   for _i in 0..num_partitions { //make vectors to hold values
        let mut x1 : Vec<usize> = Vec::new();
        for _j in 0..partition_size { //loop to push x1 to v vector
            if index != v.len() {  // if true = leftovers exist with % and not done for vector yet 
        ...
        leftover_done = 0; //reset to check for leftover vector
        xs.push(x1); //push  x1 onto xs
   }
   xs //returning results 
}
